...
def downloadPages(start_page, stop_page,start, end):
    ...
    for state in states_list:
        ...
        for date in datelist(start, end):#(2013,1,1),(2016,9,9)

            tryout = 0
            while(idx <= stop_page):

                logging.info('try obtain page: {},文书类型:判决书,裁判日期:{} TO {},法院地域:{}'.format(idx,date, date, state))
                response = getPageInfo(url, idx,date,date,state)

                content = response.content
                success = verify(content)
                if success:
                    contents = "文书类型:判决书,裁判日期:{} TO {},法院地域:{},page No.:{} \n\n".format(date, date, state,idx) + content
...
if __name__ == '__main__':
    ...
    downloadPages(1, 100, (2015,5,12), (2016,9,9))
